...
class FilterImage(torch.nn.Module):
    ...
    def forward(self, x, RLE = False):
        ...
        if RLE:
            x_rle = self.rlencode( (x_idx>0).cpu().numpy().astype(int) )
            y_rle = self.rlencode( (y_idx>0).cpu().numpy().astype(int) )
            return x_rle, y_rle
    ...
    def rldecode(self, starts, lengths, values, minlength=None):

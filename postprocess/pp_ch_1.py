...
#     cancer = (cancer > np.quantile(cancer,0.979)).astype(int)
    ...
    pp_out = pd.DataFrame({'prediction_id': prediction_id, 'cancer': cancer})
    
    #aggregate by prediction_id , i.e. by patient_laterality
    pp_out = pp_out.groupby('prediction_id')[['cancer']].agg('mean')
    
    #actual pp part
    pp_out['cancer'] = (pp_out['cancer'].values > np.quantile(pp_out['cancer'].values,0.97935)).astype(int)
